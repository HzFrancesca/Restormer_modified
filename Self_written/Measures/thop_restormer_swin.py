...
class WindowAttention(nn.Module):
    ...
    @staticmethod
    def WindowAttention_macs(module, inputs, output):
        """
        Calculates MACs within WindowAttention and adds them to module.total_ops.
        """
        x = inputs[0]
        B_, N, C = x.shape
        ...
        if module.qkv.bias is not None:
            macs_qkv += (B_ * N) * (3 * C)  # Addition for bias

        macs_attn_qk = B_ * num_heads * N * N * head_dim

        macs_relative_bias = B_ * num_heads * N * N  # Number of elements to add

        macs_attn_v = B_ * num_heads * N * head_dim * N

        macs_proj = (B_ * N) * C * C
        if module.proj.bias is not None:
            macs_proj += (B_ * N) * C  # Addition for bias

        macs_all = macs_qkv + macs_attn_qk + macs_relative_bias + macs_attn_v + macs_proj
...
if __name__ == "__main__":
    ...
    custom_ops = {WindowAttention: WindowAttention.WindowAttention_macs}
